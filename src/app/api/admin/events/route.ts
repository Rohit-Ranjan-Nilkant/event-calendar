...
export async function GET(request: NextRequest) {
  ...
  const orderBy = archived
    ? { archivedAt: "desc" as const }  // most-recently archived first
  ...
  const [events, total] = await Promise.all([
    prisma.event.findMany({
      ...
      select: {
        ...
        url: true, category: true, organizer: true,
        source: true, sourceUrl: true, isAllDay: true,
        tags: true, archived: true, archivedAt: true,
